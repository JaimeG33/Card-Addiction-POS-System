...
{
    /// <summary>
    /// Scans TCGCSV products for the selected Magic sets and loads them into dbo.NewTempCardgameInventory,
    /// then returns the inserted rows for display.
    /// </summary>
    internal sealed class AddNewMagicInventory
    {
        ...
        /// </summary>
        public AddNewMagicInventory(SqlConnectionFactory connectionFactory, Func<Task<string>> getPasswordAsync, HttpClient? http = null)
        {
...
VALUES
    (@batchPosition, @cardGameId, @setId, @cardId, @cardName, @abbreviation, @rarity, @foil, @imageURL, @mktPriceURL, @mktPrice, @amtInStock, @approved, @needsReview, @issueNotes);";
                    ...
                    foreach (var p in products)
                    {
                        ...
                        insertCmd.Parameters["@foil"].Value = DBNull.Value; // Magic: let later steps fill if needed
...
ORDER BY batchPosition, tempId;";
        ...
        private static async Task UpdateSealedRarityAsync(SqlConnection conn, int cardGameId, List<int> batchPositions, CancellationToken ct)
        {
            ...
            // Simpler, Yugioh-style normalization to avoid LOWER() syntax issues.
            // Mark as "Sealed" when rarity is null/blank and name indicates sealed product.
            cmd.CommandText = $@"
...
WHERE cardGameId = @cardGameId
  ...
  AND (
        LOWER(cardName) LIKE '%booster%'
        OR LOWER(cardName) LIKE '%box%'
        OR LOWER(cardName) LIKE '%pack%'
        OR LOWER(cardName) LIKE '%bundle%'
        OR LOWER(cardName) LIKE '%deck%'
        OR LOWER(cardName) LIKE '%kit%'
        OR LOWER(cardName) LIKE '%display%'
        OR LOWER(cardName) LIKE '%prerelease%'
        OR LOWER(cardName) LIKE '%jumpstart%'
        OR LOWER(cardName) LIKE '%draft night%'
